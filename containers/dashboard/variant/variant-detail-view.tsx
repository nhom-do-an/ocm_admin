    ...
    const onSubmit = async (values: Record<string, unknown>) => {
        ...
        try {
            await handleUpdateVariant({
                ...
                lot_management: values.lot_management,
                requires_shipping: values.requires_shipping,
                image_id: selectedImage?.id || variant.image_id,
                option1: values.option1 ?? variant.option1,
                option2: values.option2 ?? variant.option2,
                option3: values.option3 ?? variant.option3,
                type: variant.type,
            })
        } finally {
            setSaving(false)
